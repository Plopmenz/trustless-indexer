...
import { ZoneBlacklistChanged } from "../../types/trustless-management-events.js";
...
export function watchZoneBlacklistChanged(contractWatcher: ContractWatcher, storage: Storage) {
  contractWatcher.startWatching("ZoneBlacklistChanged", {
    abi: TrustlessManagementContract.abi,
    // All addresses
    eventName: "ZoneBlacklistChanged",
    strict: true,
    onLogs: async (logs) => {
      await Promise.all(
        logs.map(async (log) => {
          ...
          const event = {
            ...
            address: address,
            ...args,
          } as ZoneBlacklistChanged;

          await processZoneBlacklistChanged(event, storage);
        })
      );
    },
  });
...
export async function processZoneBlacklistChanged(event: ZoneBlacklistChanged, storage: Storage): Promise<void> {
  ...
  await storage.trustlessManagement.update((tmStorage) => {
    ...
    permission.zoneBlacklist[zone] = permissionChecker;
